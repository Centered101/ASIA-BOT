...
export async function GET(req: NextRequest) {
  ...
  async function attachStudents(rows: any[]) {
    ...
    const { data: students } = await (supabase as any)
      .from("students")
      .select("student_id, first_name, last_name, nickname, program, department, photo_url, student_phone, entry_year")
  ...
  let q = (supabase.from("change_requests") as any)
    .select("*, students!change_requests_student_id_fkey(first_name, last_name, nickname, program, department, photo_url)")
