...
export async function POST(req: NextRequest) {
  const { full_name, email, phone, department, subject, reason, desired_username, desired_password } = await req.json();
  ...
  if (desired_password) {
    if (desired_password.length < 8)        return NextResponse.json({ status: "error", message: "รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร" }, { status: 400 });
    if (!/[a-zA-Z]/.test(desired_password)) return NextResponse.json({ status: "error", message: "รหัสผ่านต้องมีตัวอักษรภาษาอังกฤษ" }, { status: 400 });
    if (!/[0-9]/.test(desired_password))    return NextResponse.json({ status: "error", message: "รหัสผ่านต้องมีตัวเลข" }, { status: 400 });
  }
  ...
  const desired_password_hash = desired_password
    ? await bcrypt.hash(desired_password, 12)
    : null;
